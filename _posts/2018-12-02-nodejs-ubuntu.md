...
subtitle:   "Step-by-step instructions for running Node.js in production"
date:       2018-12-01 10:14:00
author:     "Alessandro Segala"
header-img: "img/pie.jpg"
comments:   yes
---

I've been managing Node.js web apps in production on Linux servers for quite a few years, and I thought only now to document it publicly, hoping it can help others. There are too many ways to run Node.js apps, and I won't try to cover them all; if you're looking for a simple, straightforward way to run a Node.js web app on a Ubuntu 18.04 VM, this article is for you.
...
You can test the app locally on your machine with:
...
To start, connect to the VM (e.g. via SSH).
...
1. Packages available in the repositories of Linux distributions (including Ubuntu) tend to be quite old. Node.js evolves fast, and the maintainers of Linux distributions don't usually keep up.
...
We'll be placing the code of our apps inside the home folder of the current (non-root) user. In my case, the user is called `alessandro`:
...
Next, we're going to run the app as a background process, which is launched when the server boots and is restarted automatically after crashes.

There are many ways to daemonize a Node.js app on Linux, but my favorite one is using systemd. If you're not familiar with it, systemd is a tool present in essentially all modern Linux distributions, which has the task (among many others *– too many, according to some critics*) of managing background services.

> Another very popular option is to use a process manager like PM2. However, PM2 is itself a Node.js app, and it needs to be supervised by systemd, so at the end of the day it's the same story, but with an extra layer. Using PM2 can help in certain situations, however, such as when you frequently add or remove services.

The first step is creating a systemd unit file, which is a text file defining our Node.js application as a service. Create a file called `express_example.service` in `/etc/systemd/system/`. You can replace `express_example` with the name of your app, but note the `@` symbol at the end which we'll use to pass the port, and the `.service` extension. Copy the text below in the file `/etc/systemd/system/express_example.service` (owned by the root user):
...
[Service]
Type=simple
# Name of the system user running the service
User=alessandro
# Port the app is bound to, in this case 3000
...
# If your app's entrypoint isn't "bin/www", change this too
...
We can now enable the systemd unit so it's automatically started at boot, and start it right away:
...
Other useful systemd commands are:

````sh
# Run this if you modify the unit file on disk
...
1. It allows more efficient usage of the Node.js CPU time by letting Nginx doing TLS termination and serving static assets
...
Next, we need to configure Nginx. In this example I'm showing you the most basic configuration, without TLS; if you have multiple Node.js apps, you will want to add multiple sites listening to hostnames. Edit the file `/etc/nginx/sites-available/default` so it looks like:
...
If you open the IP (or hostname) of your server in a web browser, you should see the web app correctly running on port 80 (in addition to port 3000, which should be blocked now).
...
Node.js is single-threaded, which means that your code can only run on a single processor (*I know technically it's [not true](https://medium.com/@FloSloot/node-js-is-not-single-threaded-88928ada5838), but bear with me for simplicity*). What if you have a multi-core server?
...
sudo systemctl enable express_example@3000 express_example@3001

# Start the services right away
sudo systemctl start express_example@3000 express_example@3001
...
A good rule of thumb is to start one instance per each core your VM has (assuming you only have 1 app running on the server). In practice, this depends on the characteristics of your app, and you should run some tests to see what the optimal instance count is.
